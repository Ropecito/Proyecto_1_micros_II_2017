...
%macro	juegan 4; 'o'/'x', 'O'/'X', 'o'/'x'	
					; Mi ficha, mi ficha, ficha del otro
	                                ; Este macro comprueba el movimiento que desea hacer el usuario, con el objetivo de comprobar que sea permitido
	;&&jueganN:
	cmp	bl, %1
	je		%4
	cmp	bl, %1
...
;Este macro se encarga de hacer el tablero	
	;Perdon por esto:
                           ; * ┌ ─ │  = 3Bytes cada uno ** 
   tablero:  db   " ┌────────────────────────────────────────────────────────────────────────┐",10,
   ...
   lenTablero: equ $ - tablero
	...
	; Estos son arreglos donde se van a encontrar la fichas en el tablero
...
; Aqui se encuentran todos los posibles mensajes para errores por movimientos no permitidos
...
; Mensaje de bienvenida al usuario
	msjInicial	db 'Bienvenido al juego de tablero', 0x0A, ' Inician las "x"',0x0A
	lenMsjInicial	equ	$ - msjInicial

; Mensajes de movimientos	
...
_start:
	...
	print msjInicial, lenMsjInicial  ; Imprime mensaje de bienvenida
...
;	push	rbx
	...
	print	msjMoverDe, lenMsjMoverDe ;imprime mensaje a cual posicion desea mover
...
.leerB:	
	...
	print	msjMoverA, lenMsjMoverA; mover a x,y   ; Imprime mensaje a donde desea mover la ficha
...
;	je		infoOk
	...
	;validar sí reina!
	;si el desplazamieto es simetrico, igual en 'x' y 'y'
	;  let1 - letb = num1 - num2
...
.validar_comer:
	;;;;;;;;;;;;,,,	
; Compara si es el turno de "O" o de "X"
	cmp	r8,'o'
	je		.fComer_O
	cmp	r8,'x'
	je		.fComer_X
	;filas
	.fComer_O:					;se mueven para abajo ya que comen "O"
	...
	;jmp	.finLeerB
	.fComer_X:					;se mueven para arriba ya que comen "X"
	mov	al, byte [buffPosA]
	sub	al, 2
	cmp	al, cl
	jne	.validar_mov_normal                      ; Si no se come pasa a la etiqueda de movimiento normal
	je		.cComer_X
